...
# Han2Han-UL2-MoE Training - Host-Based Distributed TPU Pod Slice Launch
# Same as launch_han2han_ul2.sh but runs directly on the host (no Docker)
...
CONFIG_FILE="${CONFIG_FILE:-han2han-ul2-base-cooldown-v5e-eu.yaml}"
DATA_BUCKET="${DATA_BUCKET:-'gs://h2h-pt-europe-west4-a'}"
RUN_NAME="han2han_ul2_moe_$(date +%Y%m%d_%H%M%S)"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    ls -la han2han_ul2*.yaml 2>/dev/null || echo "  No han2han_ul2*.yaml files found"
    exit 1
fi

TEMP_SCRIPT="/tmp/han2han_ul2_host_training_$(basename ${CONFIG_FILE%.yaml})_$$.sh"

echo "=== Han2Han-UL2-MoE Training Launch (Host Mode) ==="
echo "TPU Pod: $TPU_POD_NAME"
echo "Zone: $TPU_ZONE"
echo "Config: $CONFIG_FILE"
echo "Data Bucket: ${DATA_BUCKET:-default}"
...
TRAIN_CMD="$HOME/venv/bin/python -u train_han2han.py --config $CONFIG_FILE"
...
echo "Starting Han2Han-UL2-MoE training on TPU worker $(hostname) (host mode)..."
...
source "$HOME/activate_training_env.sh"
cd "$HOME/Han2Han"
wandb sync --clean 2>/dev/null || true
export JAX_COMPILATION_CACHE_DIR="$HOME/jax_cache/CACHE_NAME_PLACEHOLDER"
...
if kill -0 "$TRAIN_PID" 2>/dev/null; then
    echo "Training process is running (PID $TRAIN_PID)"
else
    echo "ERROR: Training process exited immediately. Check $LOG_FILE"
    tail -20 "$LOG_FILE"
    exit 1
fi
WORKER_EOF
...
sed -i "s|CACHE_NAME_PLACEHOLDER|han2han_ul2_moe|g" "$TEMP_SCRIPT"
...
echo "Copying training script to all TPU workers..."
gcloud alpha compute tpus tpu-vm scp \
    "$TEMP_SCRIPT" "$TPU_POD_NAME":/tmp/han2han_ul2_host_training.sh \
    --zone="$TPU_ZONE" --worker=all --tunnel-through-iap

echo "Starting distributed training on all TPU workers..."
gcloud alpha compute tpus tpu-vm ssh "$TPU_POD_NAME" \
    --zone="$TPU_ZONE" --worker=all --tunnel-through-iap \
    --command='bash /tmp/han2han_ul2_host_training.sh'
...
echo "Monitor WandB: https://wandb.ai/cellikadams/han2han_base_final"
