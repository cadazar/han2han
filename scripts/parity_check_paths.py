...
def build_cfg(use_scan):
    return Han2HanConfig(
        ...
        encoder_attention_types=['mha-sliding', 'mha-sliding', 'mha-sliding', 'mha'],
        decoder_attention_types=['mha-sliding', 'mha-sliding', 'mha-sliding', 'mha'],
        ...
        use_scan_layers=use_scan, remat_policy='none', layer_pdrop=0.0,
    )
...
for use_scan in (False, True):
    key = 'scan' if use_scan else 'unrolled'
    cfg = build_cfg(use_scan)
    model = M.FlaxHan2Han(cfg, rngs=nnx.Rngs(0), dtype=jnp.float32)
    params = nnx.state(model, nnx.Param)
    n = sum(int(jnp.size(p)) for p in jax.tree_util.tree_leaves(params))
    ntensors = len(jax.tree_util.tree_leaves(params))
    ...
    results[f'{key}_n'] = n
    results[f'{key}_ntensors'] = ntensors
    results[f'{key}_logits'] = logits
    print(f"[{TAG}/{key}] params={n} tensors={ntensors} logits_sum={logits.sum():.8f}")

np.savez(f"/tmp/parity_paths_{TAG}.npz", **results)
