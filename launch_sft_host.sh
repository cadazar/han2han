...
# Han2Han SFT Fine-tuning - Host-Based Distributed TPU Pod Slice Launch
...
CONFIG_FILE="${CONFIG_FILE:-configs/it-muon-stage_1.yaml}"
DATA_BUCKET="${DATA_BUCKET:-}"
RUN_NAME="han2han_sft_$(date +%Y%m%d_%H%M%S)"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "Available SFT configs:"
    ls -la configs/sft*.yaml 2>/dev/null || echo "  No configs/sft*.yaml files found"
    exit 1
fi

TEMP_SCRIPT="/tmp/sft_host_training_$(basename ${CONFIG_FILE%.yaml})_$$.sh"

echo "=== Han2Han SFT Fine-tuning Launch (Host Mode) ==="
...
TRAIN_CMD="$HOME/venv/bin/python -u finetune_sft.py --config $CONFIG_FILE"
...
echo "Starting Han2Han SFT fine-tuning on TPU worker $(hostname) (host mode)..."
...
# kill any existing training/SFT processes
...
echo "SFT fine-tuning started with PID $TRAIN_PID"

# wait briefly and check it hasn't crashed immediately
sleep 5
if kill -0 "$TRAIN_PID" 2>/dev/null; then
    echo "SFT process is running (PID $TRAIN_PID)"
else
    echo "ERROR: SFT process exited immediately. Check $LOG_FILE"
    tail -20 "$LOG_FILE"
    exit 1
fi
WORKER_EOF
...
sed -i "s|CACHE_NAME_PLACEHOLDER|han2han_sft|g" "$TEMP_SCRIPT"
...
echo "Copying SFT script to all TPU workers..."
gcloud alpha compute tpus tpu-vm scp \
    "$TEMP_SCRIPT" "$TPU_POD_NAME":/tmp/sft_host_training.sh \
    --zone="$TPU_ZONE" --worker=all --tunnel-through-iap

echo "Starting distributed SFT on all TPU workers..."
gcloud alpha compute tpus tpu-vm ssh "$TPU_POD_NAME" \
    --zone="$TPU_ZONE" --worker=all --tunnel-through-iap \
    --command='bash /tmp/sft_host_training.sh'

rm -f "$TEMP_SCRIPT"

echo ""
echo "SFT launched on all TPU workers!"
...
echo "Monitor WandB: https://wandb.ai/cellikadams/han2han-sft"
